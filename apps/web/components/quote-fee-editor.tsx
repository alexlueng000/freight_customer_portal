'use client';
import { previewFee } from '@/lib/quote-fee-preview';
...
export function QuoteFeeEditor({ items, savedItems, currency, totalAmount, amountsByCurrency, changed, editable, disabled, onChange }: {
  items: FeeItem[];
  savedItems: FeeItem[];
  currency: string;
  totalAmount: string;
  ...
  return <>
    {editable ? <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-3">
      <p className="text-xs text-muted">费用按币种分别汇总，不自动换汇；销售金额发布后对客户可见，成本仅内部可见。海运费沿用客户需求。</p>
    ...
    <div className="overflow-x-auto">
      <table className="w-full min-w-[960px] text-left text-sm">
        <thead><tr className="border-b border-border bg-sidebar text-xs text-muted">
          {['费用名称 / 币种', '计费方式 / 箱型', '计费数量', '成本单价', '销售单价', '销售小计', '成本小计 / 毛利', ...(editable ? ['操作'] : [])].map((label) => <th key={label} className="px-3 py-3 font-semibold">{label}</th>)}
        </tr></thead>
        <tbody>{items.map((item, index) => {
          ...
          const preview = previewFee(item);
          return <tr key={item.id} className="border-b border-border">
            <td className={`${cell} min-w-44`}>{editable ? <input autoFocus={item.id.startsWith('new-') && !item.chargeName} aria-label={`${label}费用名称`} className={input} disabled={disabled} maxLength={150} placeholder="例如：文件费、提货费" value={item.chargeName} onChange={(event) => update(item.id, { chargeName: event.target.value })} /> : item.chargeName}{editable && !ocean ? <input aria-label={`${label}币种`} className={`${input} mt-2`} value={item.currency} disabled={disabled} maxLength={3} onChange={(event) => update(item.id, { currency: event.target.value.toUpperCase() })} /> : <p className="mt-2 text-xs text-muted">{item.currency}</p>}</td>
            ...
            <td className={cell}>{editable ? <input aria-label={`${item.chargeName || label}销售单价（${item.currency}）`} inputMode="decimal" maxLength={19} className={input} value={item.unitPrice} disabled={disabled} onChange={(event) => update(item.id, { unitPrice: event.target.value })} /> : money(item.unitPrice, item.currency)}</td>
            <td className={`${cell} whitespace-nowrap font-semibold`}><div className="py-2">{preview.sell === null ? '待填写' : money(preview.sell, item.currency)}</div>{changed ? <span className="text-xs text-muted">未保存预览</span> : null}</td>
            <td className={`${cell} whitespace-nowrap`}><div>成本：{preview.cost === null ? '待成本确认' : money(preview.cost, item.currency)}</div><div className="mt-1">毛利：{preview.profit === null ? '待成本确认' : money(preview.profit, item.currency)}</div></td>
        ...
        <tfoot><tr className="bg-primary/5 font-semibold"><td colSpan={editable ? 7 : 6} className="px-3 py-4 text-right">报价总额（分币种）</td><td className="px-3 py-4 text-right text-primary">{changed ? '保存后重新计算' : quoteAmounts({ currency, totalAmount, amountsByCurrency })}</td></tr></tfoot>
      </table>
    </div>
