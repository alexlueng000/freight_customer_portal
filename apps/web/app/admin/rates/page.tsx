...
function ImportRateDialog({ apiFetch, onClose, onCompleted }: { apiFetch: (input: RequestInfo | URL, init?: RequestInit) => Promise<Response>; onClose: () => void; onCompleted: () => void }) {
  const [file, setFile] = useState<File | null>(null); const [importJob, setImportJob] = useState<RateImport | null>(null); const [error, setError] = useState<string | null>(null); const [submitting, setSubmitting] = useState(false); const [downloading, setDownloading] = useState(false);
  useEffect(() => {
  ...
  const downloadTemplate = async () => { setDownloading(true); setError(null); try { const response = await apiFetch('/api/v1/rates/import-template'); if (!response.ok) throw new RateApiError('模板下载失败，请稍后重试。'); const url = URL.createObjectURL(await response.blob()); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'rate-import-template.xlsx'; anchor.click(); URL.revokeObjectURL(url); } catch (caught) { setError(toRateError(caught).message); } finally { setDownloading(false); } };
  const busy = submitting || importJob?.status === 'PENDING' || importJob?.status === 'PROCESSING';
  return <div aria-labelledby="import-rate-title" aria-modal="true" className="fixed inset-0 z-50 flex items-start justify-end bg-foreground/30" role="dialog"><button aria-label="关闭导入表单" className="absolute inset-0" disabled={busy} onClick={onClose} type="button" /><div className="relative h-full w-full max-w-2xl overflow-y-auto border-l border-border bg-surface shadow-xl"><div className="flex items-start justify-between border-b border-border px-5 py-4"><div><h2 className="text-lg font-semibold" id="import-rate-title">Excel 导入运价</h2><p className="mt-1 text-sm text-muted">使用标准模板；整批校验通过后才会写入数据库。</p></div><button aria-label="关闭" className="grid size-9 place-items-center rounded border border-border disabled:opacity-40" disabled={busy} onClick={onClose} type="button"><X className="size-4" /></button></div><div className="space-y-5 p-5">
    <section className="rounded border border-border bg-background p-4"><h3 className="text-sm font-semibold">1. 下载并填写模板</h3><p className="mt-1 text-sm text-muted">模板使用中文表头并包含多条样例；一行代表一个箱型价格，相同运价编号的多行会合并为一条运价。</p><button className={`${secondaryButton} mt-3`} disabled={downloading || busy} onClick={() => void downloadTemplate()} type="button"><Download className="size-3.5" /> {downloading ? '下载中…' : '下载标准模板'}</button></section>
    <section className="rounded border border-border bg-background p-4"><h3 className="text-sm font-semibold">2. 上传工作簿</h3><input accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="mt-3 block w-full rounded border border-border bg-surface p-2 text-sm file:mr-3 file:rounded file:border-0 file:bg-sidebar file:px-3 file:py-1.5 file:text-sm file:font-medium" disabled={busy} onChange={(event) => { setFile(event.target.files?.[0] ?? null); setError(null); setImportJob(null); }} type="file" /><p className="mt-2 text-xs text-muted">仅支持 .xlsx，最大 5 MB、最多 5,000 个数据行。已有 rateNo 会导致整批失败。</p></section>
    {error ? <div className="rounded border border-danger/20 bg-danger/10 px-3 py-2 text-sm text-danger">{error}</div> : null}
    {importJob ? <section className={`rounded border p-4 ${importJob.status === 'FAILED' ? 'border-danger/20 bg-danger/5' : importJob.status === 'COMPLETED' ? 'border-success/20 bg-success/5' : 'border-border bg-background'}`}><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">导入结果</h3><StatusBadge tone={importJob.status === 'COMPLETED' ? 'success' : importJob.status === 'FAILED' ? 'danger' : 'info'}>{importJob.status === 'PENDING' ? '等待处理' : importJob.status === 'PROCESSING' ? '正在处理' : importJob.status === 'COMPLETED' ? '导入完成' : '导入失败'}</StatusBadge></div><dl className="mt-3 grid grid-cols-3 gap-3 text-sm"><div><dt className="text-muted">总行数</dt><dd className="mt-1 font-semibold">{importJob.totalRows}</dd></div><div><dt className="text-muted">成功</dt><dd className="mt-1 font-semibold text-success">{importJob.successRows}</dd></div><div><dt className="text-muted">失败</dt><dd className="mt-1 font-semibold text-danger">{importJob.failedRows}</dd></div></dl>{importJob.errorMessage ? <p className="mt-3 text-sm text-danger">{importJob.errorMessage}</p> : null}{importJob.errors?.length ? <div className="mt-3 max-h-64 overflow-auto rounded border border-border bg-surface"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-sidebar"><tr><th className="px-3 py-2">行</th><th className="px-3 py-2">字段</th><th className="px-3 py-2">错误</th></tr></thead><tbody>{importJob.errors.map((item, index) => <tr className="border-t border-border" key={`${item.row}-${item.field}-${index}`}><td className="px-3 py-2">{item.row}</td><td className="px-3 py-2 font-mono">{item.field}</td><td className="px-3 py-2">{item.message}</td></tr>)}</tbody></table></div> : null}</section> : null}
    <div className="flex justify-end gap-2 border-t border-border pt-4"><button className="h-9 rounded border border-border px-4 text-sm font-semibold disabled:opacity-40" disabled={busy} onClick={onClose} type="button">关闭</button><button className="h-9 rounded bg-primary px-4 text-sm font-semibold text-surface disabled:opacity-40" disabled={!file || busy || importJob?.status === 'COMPLETED'} onClick={() => void upload()} type="button">{submitting ? '上传中…' : busy ? '处理中…' : importJob?.status === 'FAILED' ? '重新上传' : '开始导入'}</button></div>
  </div></div></div>;
}
