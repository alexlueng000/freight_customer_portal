...
for (const width of [1440, 390]) {
  test(`customer booking keeps SO and transport together at ${width}px`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByText('货代已收到订舱确认单，正在核对', { exact: false })).toBeVisible();
    await expect(page.getByRole('button', { name: /下载 订舱确认单/ })).toHaveCount(0);
    published = true;
    await page.reload();
    await expect(page.getByText('订舱确认单SO-PILOT · V1')).toBeVisible();
    await expect(page.getByText('集装箱进港截止：', { exact: false })).toBeVisible();
    await expect(page.getByRole('link', { name: '查看报价与服务条款 →' })).toHaveAttribute('href', '/portal/quotes/q1#service-terms');
    const download = page.waitForEvent('download');
    await page.getByRole('button', { name: /下载 订舱确认单：pilot-so.pdf/ }).click();
