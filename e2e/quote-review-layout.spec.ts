...
for (const width of [1440, 390]) {
  test(`quote review fields and collapsible source work at ${width}px`, async ({ page }, testInfo) => {
    ...
    let review: { validUntil: string; customerTerms: string; internalNote: string; plannedSailingDate: string | null } = { validUntil: '2026-10-02', customerTerms: '', internalNote: '', plannedSailingDate: null };
    let unitPrice = '1300';
    ...
    await page.route('**/api/v1/**', async (route) => {
      ...
      if (path.endsWith('/auth/refresh')) return route.fulfill({ json: {
        ...
        user: { id: 'sales', tenantId: 'demo', tenantCode: 'DEMO', tenantName: 'Demo', tenantBrandName: 'Demo',
          displayName: '测试销售', email: 'sales@example.test', userType: 'INTERNAL', roles: ['SALES'], permissions: ['quote.manage', 'rate.read', 'customer.read'] },
      } });
      ...
      if (path.endsWith('/prices')) {
        ...
        review = { validUntil: body.validUntil, customerTerms: body.customerTerms, internalNote: body.internalNote, plannedSailingDate: body.plannedSailingDate };
      ...
      if (path.endsWith('/admin/quotes/layout-test')) return route.fulfill({ json: {
        ...
        customer: { id: 'customer-test', name: '测试客户' }, bookings: [], sentAt: null, sentBy: null, priceOverrideReason,
    ...
    await expect(page.getByRole('link', { name: '测试客户', exact: true })).toHaveAttribute('href', '/admin/customers/customer-test');
    await expect(page.getByRole('link', { name: 'RATE-TEST', exact: true }).first()).toHaveAttribute('href', '/admin/rates?search=RATE-TEST');
    const fields = page.getByRole('region', { name: '报价编辑' });
    await expect(fields).toBeVisible();
    const sailingDate = fields.getByLabel('拟参加船期（选填）');
    await expect(sailingDate).toHaveValue('');
    await sailingDate.fill('2026-10-05');
    ...
    await page.reload();
    await expect(sailingDate).toHaveValue('2026-10-05');
    await sailingDate.fill('2026-10-08');
    ...
    await expect(sailingDate).toHaveValue('2026-10-08');
    await sailingDate.fill('');
    await fields.getByRole('button', { name: '保存报价', exact: true }).click();
    await expect(fields.getByRole('button', { name: '保存报价', exact: true })).toBeDisabled();
    expect(review.plannedSailingDate).toBeNull();
    await page.reload();
    await expect(sailingDate).toHaveValue('');
    ...
    await page.route('**/api/v1/rates?**', (route) => route.fulfill({ json: {
      items: [], pagination: { page: 1, pageSize: 20, total: 0, totalPages: 0 },
    } }));
    const rateSearch = page.waitForRequest((request) => {
      const url = new URL(request.url());
      return url.pathname === '/api/v1/rates' && url.searchParams.get('search') === 'RATE-TEST';
    });
    await page.getByRole('link', { name: 'RATE-TEST', exact: true }).last().click();
    await rateSearch;
    await expect(page).toHaveURL(/\/admin\/rates\?search=RATE-TEST$/);
    await expect(page.getByPlaceholder('搜索运价编号、港口、供应方或合约号')).toHaveValue('RATE-TEST');
  });
}
