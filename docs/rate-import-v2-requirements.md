...
1. 新增 V2 宽表导入模板：一条运价一行。
2. 系统自动把横向箱型价格拆成现有 `Rate + RatePrice[]` 数据结构。
3. 新模板默认继承“运价币种”，不再要求每个箱型重复填写“价格币种”。
4. 支持附加费结构化 Excel 导入。
5. 保持旧版长表模板可继续导入，不能破坏现有数据和接口。
6. 导入前完成字段校验，不允许部分错误数据悄悄入库。
7. 导入结果必须明确展示：成功数、失败数、警告数、具体错误行。

### P1（可在 P0 完成后继续）

1. 导入预览页：确认后再正式写入数据库。
2. 支持“仅新增 / 更新已有运价”的导入模式。
3. 支持模板字段 Mapping，为以后导入不同船司/供应商 Excel 做准备。
