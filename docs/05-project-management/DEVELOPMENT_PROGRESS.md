...
## 2026-09-18 报价申请与审核优化

- 客户报价申请改为独立页面，新增工厂预计装货日期、船期冲突提醒，提派地点无条件必填；增加二次确认、错误字段定位和分区视觉优化。
- 后台报价价格、有效期、条款和内部备注统一在页面编辑与保存；改价时原子保存，未保存修改先提示保存再发布。
- 新增 Quote 可空 DATE 字段迁移，已应用本机数据库。按用户要求，最终版本未运行测试，待用户统一验收。
- 完整改动、历史验证限制和验收清单：[当天优化记录](./2026-09-18_报价申请与审核优化记录_CN.md)。
