...
根据 2026-09-06 实际业务反馈，客户向销售申请正式报价时必须同步提交：

- 一种或多种货物明细；每种货物分别填写品名、毛重（kg）和特殊要求（选填）；
- 发货地、收货地（选填；勾选对应头程/尾程物流后必填）；
- 需要货代办理的服务：头程物流、始发国清关、目的国清关、尾程物流，可多选。

货物信息区右上角提供“添加货物”，每条货物可单独删除，但至少保留一条；一次报价最多 50 种货物。这些字段属于本次客户运输需求，不是对 Rate 已有字段的重复录入。新建 Quote 必须保存需求快照，并在客户 Quote 详情和销售审核页展示。历史 Quote 可保持为空。
