...
## 13. 界面中文用语约定（2026-09-15）

客户门户和运营后台的标签、按钮与提示统一使用以下用语：

| 业务字段或英文术语 | 界面用语 |
| --- | --- |
| POL / POD | 起运港 / 目的港 |
| ETD / ETA | 预计开船时间 / 预计到港时间 |
| Carrier / Service | 船司 / 航线服务 |
| Incoterm | 贸易条款 |
| Quote Request / Formal Quote | 报价申请 / 正式报价 |
| Booking / Basic Shipment | 订舱 / 出运记录 |
| Vessel / Voyage | 船名 / 航次 |

运价列表的「开船日」兼容具体日期和「每周五」「每月15日」等周期船期，不代表实际开船时间。确认 SO 中的预计开船时间也不等于实际开船时间。

这些约定仅用于界面显示。API 字段、状态枚举、业务编号、箱型与币种代码保持原有含义；导入备注的 `Schedule:` 等解析标记继续兼容，不替换已有数据库内容。
